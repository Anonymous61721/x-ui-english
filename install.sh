...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/Anonymous61721/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/Anonymous61721/x-ui-english/main/config/version >/dev/null 2>&1)
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/Anonymous61721/x-ui-english/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    wget -N --no-check-certificate https://raw.githubusercontent.com/Anonymous61721/x-ui-english/main/x-ui.sh -O /usr/bin/x-ui
...
install_xui() {
    ...
    echo -e "------------------------------------------------------------------------------"
    echo -e "rezaezati   - https://github.com/Anonymous61721"
    echo -e "--------------------------------------------------------------------------------"
    show_login_info
    echo -e ""
